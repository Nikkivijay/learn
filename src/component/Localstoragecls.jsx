...
export class Localstoragecls extends Component {
  ...
  constructor() {
    ...
    this.state = {
      obj: {},
      blank : {id: 0 , fname: "", lname: "", email: "", password: "", city: "", date: "", gender: "", hobby: []},
      count : 1,
      hby : [],
      arry : []
    }
    // this.hby = this.state.hobby ? this.state.hobby : []
    // this.state.obj = {...this.state.obj}

    this.state.obj = {...this.state.blank};
  ...
  componentDidMount(){
    this.setState({arry : JSON.parse(localStorage.getItem('arry')) || []});
    this.setState({count : Number(localStorage.getItem('count')) || 1});
  }

  componentDidUpdate(){
    localStorage.setItem('arry',JSON.stringify(this.state.arry));
  ...
  saveData = (e) => {
    ...
    if(this.state.obj.id == 0){
      ...
      this.state.arry.push(this.state.obj);
    }
    else{
      let index = this.state.arry.findIndex(x => x.id == this.state.obj.id)
      this.state.arry.splice(index,1,this.state.obj)
      console.log(index)
    }
  ...
  deleteuser = (id) => {
    let index = this.state.arry.findIndex(x => x.id == id)
    let splice = this.state.arry.splice(index,1)
    this.setState({...this.state.obj, splice})
      // console.log(index)
  }

  edituser = (id) =>{
    let index = this.state.arry.find(x => x.id == id)
...
export default Localstoragecls